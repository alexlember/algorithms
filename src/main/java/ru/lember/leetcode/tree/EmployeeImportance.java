...
public class EmployeeImportance {
    ...
    public int getImportance(List<Employee> employees, int id) {
        ...
        return subImportance(id, map);
    }

    private int subImportance(int id, Map<Integer, Employee> map) {
        Employee searched = map.get(id);
        if (searched == null) {
            return 0;
        }

        int sum = searched.importance;

        if (searched.subordinates != null) {
            sum += (int) searched.subordinates.stream()
                    .map(_id -> subImportance(_id, map))
